...
from utils import prepare_mlp, prepare_optim_and_scheduler, padding
...
def prepare_model(args):
    ...
    mlp1 = prepare_mlp(model.config.hidden_size*3)
    mlp2 = prepare_mlp(model.config.hidden_size*3)
    return [model, mlp1, mlp2]
...
def run_para_model(mlp, outputs, dropout_p, ds, ds2, attn, train):
    m = nn.LogSoftmax(dim=-1)
    dropout = nn.Dropout(dropout_p)
    pooled_output = mean_pooling(outputs, attn)
    all_normalized = []
    all_indexes = []
    start, end = 0, 0
    for i in range(len(ds)):
        ...
        curr = pooled_output[start:end]
        t = []
        for i in d2:
            l = d2[i]
            new_t = torch.sum(curr[l[0]:l[-1]+1], dim=0)
            t.append(new_t)
        pooled_output = torch.stack(t)
        if train:
            pooled_output = dropout(pooled_output)
        start = end
        combs = torch.combinations(torch.arange(pooled_output.size(0), device=device))
        C = len(combs)
        paired = pooled_output[combs,:]
        diff = torch.abs(paired[:,0] - paired[:,1])
        pairs = torch.cat([paired.view(C,-1), diff], dim=-1).view(C, -1)
        outs = mlp(pairs).view(-1)
        outs = m(outs)
        all_normalized.append(outs)
        all_indexes.append(combs)
    return all_normalized, all_indexes

def get_para_selected(paras, indexes, kp, mode):
    all_top_pouts = []
    all_top_vals = []
    for p, idx in zip(paras, indexes):
        if mode == "topk":
        ...
        all_top_vals.append(top_pvals)
        all_top_pouts.append(idx[top_pouts])
    return all_top_pouts, all_top_vals

def run_sent_model(mlp, tok, input_ids, lm_outputs, ds, num_s, top_pouts, train):
    m = nn.LogSoftmax(dim=-1)
    ...
    indexes = []
    start, end = 0, 0
    for d, n, top_pout in zip(ds, num_s, top_pouts):
        end += n
        ...
        curr_indexs = []
        for i, j in top_pout:
            paired = torch.cartesian_prod(torch.arange(len(group_by_p[i]), device=device), torch.arange(len(group_by_p[j]), device=device))
            curr_indexs.append(paired)
            paired_values = torch.cat([group_by_p[i][paired[:, 0]].unsqueeze(1), group_by_p[j][paired[:, 1]].unsqueeze(1)], dim=1)
            diff = torch.abs(paired_values[:,0] - paired_values[:,1])
            paired_values = torch.cat([paired_values.view(paired_values.size(0),-1), diff], dim=-1).view(paired_values.size(0), -1)
            outs = mlp(paired_values).view(-1)
            outs = m(outs)
            curr_outs.append(outs)
        groups.append(curr_outs)
        indexes.append(curr_indexs)
        start = end
    return groups, indexes

def get_sent_selected(sents, indexes, ks, mode):
    all_top_souts = []
    all_top_vals = []
    for s, idx in zip(sents, indexes):
        if mode == "topk":
            ...
            raise NotImplementedError
        top_souts = [j[sout] for j, sout in zip(idx, top_souts)]
        all_top_souts.append(top_souts)
        all_top_vals.append(top_svals)
    return all_top_souts, all_top_vals

def get_para_sent(pvals, svals):
    all_ps_vals = []
    for para, sent in zip(pvals, svals):
        ps_vals = [curr_p + curr_s for curr_p, curr_s in zip(para, sent)]
...
def pad_answers(tokenizer, contexts, raw_answers, topkp, topks):
    ...
    for cont, cur_tokp, cur_topks in zip(contexts, topkp, topks):
        l = 0
        for para, sent in zip(cur_tokp, cur_topks):
            pi, pj = para
            curr = []
            for s in sent:
                si, sj = s
                first = cont[pi][si]
                second = cont[pj][sj]
                idx = second.index(tokenizer.sep_token_id)
                second = second[idx+1:]
                curr.append(first+second)
            out_cs += curr
            l += len(curr)
        lens.append(l)
...
def run_model(batch, layers, answer_model, tokenizer, answer_tokenizer, max_p, reg_coeff, t, mode, topkp, topks, beam=2, train=True):
    ...
    pouts, pindexes = run_para_model(layers[1], lm_outs, layers[0].config.hidden_dropout_prob, batch["ds"], batch["ds2"], batch["attention_mask"], train=train)
    top_pouts, para_vals = get_para_selected(pouts, pindexes, topkp, mode=mode)
    souts, sindexes = run_sent_model(layers[2], tokenizer, batch["input_ids"], lm_outs, batch["ds"], batch["num_s"], top_pouts, train=train)
    top_souts, sent_vals = get_sent_selected(souts, sindexes, topks, mode=mode)
    para_sent = get_para_sent(para_vals, sent_vals)
